...
public class GameArea {
    ...
    public void movePlayer(String direction) {
        if (direction.equals("left") && (!playerCollisionLogic.isCollisionLeft())) {
            playerBlock.moveX(direction);
        }
        if (direction.equals("right") && (!playerCollisionLogic.isCollisionRight())) {
            playerBlock.moveX(direction);
        }
        if (direction.equals("down") && (!playerCollisionLogic.isCollisionDown())) {
            playerBlock.moveDown();
        } else if (direction.equals("down") && (playerCollisionLogic.isCollisionDown())) {
            releaseBlocks();
            seekBlockStreaks();    
            if (!gameOver()) {
                playerBlock.respawn(eglibleRespawn());
